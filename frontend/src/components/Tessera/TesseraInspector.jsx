...
function Field({ label, what, value, mono = true, accent = 'blue', truncate = false }) {
  ...
  // Copy always writes the FULL value, never the truncated display string.
  const copy = () => {
    if (!full) return
    navigator.clipboard?.writeText(full)
    setCopied(true); setTimeout(() => setCopied(false), 1200)
  }
  // Truncate only what's shown on screen.
  const display = truncate && full.length > 80 ? full.slice(0, 80) + '…' : full
  return (
    <div className="py-3 border-b border-gray-50 dark:border-neutral-800/50 last:border-0">
      <div className="flex items-center justify-between mb-1">
        ...
        {full && <button onClick={copy} className="text-[10.5px] text-gray-400 hover:text-gray-600 dark:hover:text-neutral-300">{copied ? 'copied' : 'copy'}</button>}
      </div>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4">
      <Group title="Identity & timestamp" subtitle="Who signed this, and when — provable against the CA">
        ...
        <Field label="Vendor certificate" what="The signer's X.509 identity, issued by the Attestr CA." value={vendorCert} truncate accent="blue"/>
        <Field label="RFC 3161 timestamp token" what="A trusted clock's countersignature over the Merkle root — proves submission time and prevents backdating." value={b.rfc3161_timestamp_token} truncate accent="amber"/>
      </Group>
      ...
      <Group title="Integrity — Merkle + ECDSA" subtitle="Binds every answer to the signer; one byte changes the root">
        ...
        <Field label="ECDSA signature" what="The vendor's secp256r1 signature over the Merkle root. Only their private key could have produced it." value={b.ecdsa_signature} truncate accent="violet"/>
      ...
      <Group title="Confidentiality — hybrid encryption" subtitle="End-to-end; the server never sees plaintext">
        <Field label="Ephemeral public key" what="A throwaway ECDH key for this submission only. Provides perfect forward secrecy — destroyed after use." value={b.ephemeral_public_key} truncate accent="emerald"/>
        ...
        <Field label="Encrypted payload" what="The answers, encrypted for the auditor's key. Unreadable without the auditor's private key." value={b.encrypted_payload} truncate accent="emerald"/>
      </Group>
